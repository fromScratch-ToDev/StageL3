...
function MenuMobile(props : {handleClick : () => void}) {
    ...
    if(menu_galeries){
        return(
            <div className="fixed inset-0 z-10 bg-primary overflow-clip">
                ...
                <ul className="list-none pl-5 space-y-3 font-bold">
                    {galeries.map((galerie) => (
                        <li key={galerie.nom_fr} ><Link href={`/galerie?nom=${galerie.nom_fr}`}>{lang === "FR" ? galerie.nom_fr : galerie.nom_en}</Link></li>
                    ))}
                </ul>
            </div>
        )
    ...
    return (
        <div className="fixed inset-0 z-10 bg-primary overflow-clip">
            ...
            <nav className="list-none pl-5 text-2xl space-y-3 font-bold">
                <li><Link href="/">{lang === "FR" ? "Accueil" : "Home"}</Link></li>
                <li onClick={()=>set_menu_galeries(!menu_galeries)}> {lang === "FR" ? "Galeries" : "Galleries"} </li>   
                <li><Link href="/expositions">{lang === "FR" ? "Expositions" : "Exhibitions"}</Link></li>
                <li><Link href="/biographie">{lang === "FR" ? "Biographie" : "Biography"}</Link></li>
